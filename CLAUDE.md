...
[.github/workflows/pr-preview.yml](.github/workflows/pr-preview.yml) publishes the Blazor app from every open PR to GitHub Pages so it can be tested before merge, and comments the link on the PR.

- Previews live at `https://ryangano.github.io/Catandomizer/pr-preview/pr-<n>/`, with a landing page listing all live previews at the Pages root. Both are served from the `gh-pages` branch, which this workflow owns end to end — nothing else should commit to it.
- The landing page is regenerated by [.github/scripts/build-preview-index.sh](.github/scripts/build-preview-index.sh) from whatever `pr-preview/pr-*` directories exist, on both deploy and cleanup, so it never needs separate state.
- The workflow rewrites `<base href="/" />` in the published `index.html` to the preview subpath and copies `index.html` to `404.html` (Pages has no SPA rewrite, and `Index.razor` serves `@page "/{Code}"` deep links). It also writes `.nojekyll` at the branch root, without which Jekyll swallows `_framework/`.
- Previews call the **production** service, so PRs that change `CatandomizerService` are not testable there. Every preview shares the `https://ryangano.github.io` origin, so the one CORS entry in `Program.cs` covers all of them.
- App Service deployment slots are not an option: the plan (`ASP-AWorldChangerCatandomizerServic-ba09`) is Free (F1), and slots need Standard or above.
- One-time setup: repo **Settings → Pages → Source: Deploy from a branch → `gh-pages` / `/ (root)`**. Fork PRs are skipped — their token cannot push to `gh-pages`.
